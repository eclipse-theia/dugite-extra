...
import { FileStatus, WorkingDirectoryFileChange, WorkingDirectoryStatus, IAheadBehind, IStatusResult } from '../model/status';
import { parsePorcelainStatus } from '../util/status-parser';
import { DiffSelection, DiffSelectionType } from '../model/diff';
import { git } from '../util/git';

/**
 * Maps the raw status text from Git to an app-friendly value
 * shamelessly borrowed from GitHub Desktop (Windows)
 */
function mapStatus(rawStatus: string): FileStatus {
 ...
 */
export async function status(repositoryPath: RepositoryPath): Promise<IStatusResult> {
    const path = RepositoryPath.getPath(repositoryPath);
    const result = await git(['status', '--untracked-files=all', '--branch', '--porcelain=2', '-z'], path, 'getStatus')
